...
void Crash::crashHandler( int /*signal*/ )
{
	...
	// we are the child process (the result of the fork)
		std::cout << "KLudoteca is crashing...\n";

		QString path = "/tmp/backtrace.kl";
	QString subject = "0.1a";
	QString body = i18n(
		"KLudoteca has crashed! We're terribly sorry about this :(\n\n"
		"But, all is not lost! You could potentially help us fix the crash. "
		"KLudoteca has attached a backtrace that describes the crash, so just click send, "
		"or if you have time, write a brief description of how the crash happened first.\n\n"
		"Many thanks.\n\n" );

	body += "Build date: " __DATE__ "\n"
...
//                     "thread apply all bt\n";
	...
	/// analyze usefulness
	const QString fileCommandOutput = runCommand( "file `which kludoteca`" );
...
CrashHandlerWidget::CrashHandlerWidget()
{
    ...
    layout->add( new QLabel( /*i18n*/(
            "<p>" "KLudoteca is crashed! "
            "<p>" "However you now have an opportunity to help us fix this crash so that it doesn't "
                  "happen again! Click <b>Send Email</b> and KLudoteca will prepare an email that you "
